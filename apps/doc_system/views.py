...
_KNOWLEDGE_BASE = [
    ...
    {
        "keywords": ["processamento interrompido", "contate o administrador", "interrompido antes da conclusao", "processamento travou", "travou no meio", "parou no meio", "worker timeout", "ficou preso", "processamento orfao", "nao terminou", "parou de processar", "documento nao processou", "timeout do servidor", "lote grande demais", "muitos documentos de uma vez demora", "processamento lento demais"],
        "resposta": "**\"Processamento interrompido antes da conclusão. Contate o administrador do sistema.\"** é uma mensagem genérica: o servidor tem um limite de 10 minutos por execução manual (\"Executar\"). Se um lote de documentos passa desse tempo — geralmente porque cada documento individual está demorando muito (ex.: PDF grande, modelo mais lento) — o servidor interrompe a execução no meio, e o sistema marca como erro ~5 minutos depois, quando percebe que nada mais está acontecendo.\n\n📋 Os documentos já processados **ficam salvos normalmente** (confira em \"Ver tokens por documento\") — só os que ainda não tinham rodado ficam pendentes. Basta clicar em **Executar** de novo no mesmo agente: os já processados são ignorados automaticamente (regra de duplicidade), só os que faltaram são retomados.\n\n💡 **Para evitar que aconteça de novo:** em vez de clicar em Executar com muitos documentos de uma vez, ative a **rotina automática** desse agente (ver Administrador > Rotina automática) — ela processa em lotes pequenos e seguros, automaticamente, sem nunca esbarrar nesse limite de tempo.",
        "link": "/doc-system/processamentos/",
    },
    {
    ...
    {
        "keywords": ["reduzir raciocinio", "reduzir custo de raciocinio", "thinking budget", "custo de pensamento", "tokens de pensamento", "thoughts token", "reduzir custo ia raciocinio", "desligar raciocinio ia", "gemini 2.5 pro", "budget 0 is invalid", "thinking mode", "modelo nao aceita reducao"],
        "resposta": "**Reduzir custo de raciocínio da IA:** opção por agente (em Gerenciar agentes) que pede para a IA responder direto, sem gastar tokens com um rascunho interno (\"thinking\") antes da resposta final — um custo cobrado mas que nunca aparece pro usuário.\n\n📉 Medido em produção: reduziu o total de tokens em cerca de 22-27% num checklist de análise de edital, sem deixar nenhum item sem resposta.\n\nSó tem efeito em modelos que suportam esse ajuste. **O `gemini-2.5-pro` não aceita desligar o raciocínio** (só funciona em thinking mode) — o sistema já detecta isso automaticamente e nem tenta pedir a redução nesse modelo, para não desperdiçar uma chamada de IA inteira em cada documento. Pode deixar o toggle ligado sem medo, mesmo usando esse modelo. Vem desligada por padrão — como a IA passa a \"pensar menos\" nos modelos que aceitam, vale revisar as respostas depois de ligar, principalmente em itens que exigem julgamento mais fino (ex.: habilitação em editais).",
        "link": "/doc-system/gerenciar-agentes/",
    },
    {
